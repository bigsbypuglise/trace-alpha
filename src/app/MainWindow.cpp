...
MainWindow::MainWindow() {
    ...
    connect(&playTimer_, &QTimer::timeout, this, [this]() {
        ...
        if (frameCycleClock_.isValid()) {
            ...
            // WHAT THE SINK DID WHILE THE UI THREAD WAS NOT RUNNING.
            //
            // This decides whether the picture jumping on resume is a defect or
            // a requirement, and the two answers are opposite:
            //
            //   audio kept playing -> the jump is CORRECT. It is how sync is
            //       maintained, and freezing the video clock instead would leave
            //       the picture permanently behind the sound.
            //   audio stalled too  -> the jump is removable. The clock can be
            //       re-anchored across the gap with nothing to catch up to.
            //
            // processedUSecs is the term that can answer it, because it counts
            // bytes handed to the DEVICE and the device is pulled by its own
            // MMCSS thread rather than by this one. Compared against the wall
            // gap the tick just measured:
            //
            //   proc ~= period          audio played straight through
            //   proc ~= 0               audio stopped with the thread
            //   proc ~= device buffer   the device drained what it had, then
            //                           underran -- the interesting middle case,
            //                           and the one that fits "picture and sound
            //                           freeze together" while the clock still
            //                           advances
            //
            // Only read under the knob: stats() is not free and the shipping
            // tick must not pay for a diagnostic.
            if (tickLogEnabled()) {
                const auto as = audio_.stats();
                QString audioPart;
                if (!audio_.isPlaying()) {
                    audioPart = QStringLiteral(" audio=idle");
                } else {
                    const double procDeltaMs =
                        (tickLogPrevProcUSecs_ >= 0)
                            ? static_cast<double>(as.processedUSecs - tickLogPrevProcUSecs_) / 1000.0
                            : -1.0;
                    const double clkDeltaS =
                        (tickLogPrevClockS_ >= 0.0) ? (as.clockSeconds - tickLogPrevClockS_) : -1.0;
                    audioPart = QString(" audio proc=%1ms clk=%2s ring=%3ms under=%4 silence=%5B state=%6")
                        .arg(QString::number(procDeltaMs, 'f', 2))
                        .arg(QString::number(clkDeltaS, 'f', 3))
                        .arg(QString::number(as.bufferedMs, 'f', 1))
                        .arg(as.underruns)
                        .arg(as.silenceBytes)
                        .arg(as.sinkState);
                }
                if (tickFrameDurationMs_ > 0.0
                    && lastPeriodMs_ > tickFrameDurationMs_ * kTickLateFactor) {
                    tickLogLine(QString("t=%1s period=%2ms handler=%3ms budget=%4ms sizemove=%5%6")
                ...
                tickLogPrevProcUSecs_ = audio_.isPlaying() ? as.processedUSecs : -1;
                tickLogPrevClockS_ = audio_.isPlaying() ? as.clockSeconds : -1.0;
            }
        }
