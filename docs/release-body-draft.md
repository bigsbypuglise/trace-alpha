...
> The one item left on the checklist at the bottom is the owner's look at the interface.
>
> **This draft supersedes the 2026-08-18 one**, which described the painted strip blur — a
> feature the owner's feedback pass has since removed — and a ten-control strip that is now
> eight. Everything below is written against the current build, 47 commits past `v0.2.0-beta.1`.
...
Trace is running takes effect immediately. (An earlier build blurred the video behind the strip
instead; the sweep measured translucency-over-blur as strictly less legible than either alone,
and the blur was removed.)
...
- **Mixed-monitor DPI is validated at 100% and 150% only.** Other scale factors, three-plus
  displays, hot-plug and live scale changes are a known, accepted gap — the second display is
  disconnected and no further multi-display work is planned until a report arrives.
...
1. ~~**Owner decides the version.**~~ **DONE — `v0.3.0-beta.1`** (owner, 2026-08-18). The minor
   ...
   release. Verified against the built binary at `86f1186` rather than the source: `0.3.0`
   present, `0.2.0` absent, and all three stage literals present (`(beta)` in the build
   identity, `Beta.` in the About dialog, `beta - issue report` in the mail subject) — noting
   Qt stores `tr()` strings as UTF-8 while the CMake define lands as UTF-16, so the search ran
   in both encodings.
2. ~~**Push and confirm CI green.**~~ **DONE at `86f1186`, 2026-08-19, all five verification
   steps read individually**: dependency gate `all DLLs import only Windows system libraries` ·
   `FFmpeg detected` + `Audio dependencies detected` · `6 required files present, 95.4 MB
   total` · **`renderer=d3d11 fellback=0 planar=1`** (the hardware path, not WARP) ·
   `OK - 11 shapes x 4 scale factors`. Note CI had been **red for three runs** before this:
   linking Trace as a GUI app (owner item 18) broke both selftest steps, because pwsh does not
   wait for a bare GUI-app invocation — fixed in the workflow with `Start-Process -Wait`, which
   reads the exit code off the process object.
3. ~~**One Windows setting flip.**~~ **DONE — closed on hardware** (owner, 2026-08-18), and the
   mechanism survived item 8: the transparency setting now gates the strip's resting alpha
   instead of the deleted blur, live via `WM_SETTINGCHANGE`.
4. **An owner look at the new interface on the machine.** Every figure in this release is
   measured; nobody has signed off how the finished interface *feels* end to end. The last five
   interface phases were each signed off by eye as well as by number, and this is a bigger
   visual change than any of them. The feedback pass means the owner has seen most of it — but
   the resting translucency (item 8) landed last and has not been seen settled.
