...
- **Speculative reverse-cache fill is small on seek walks** (July 2026): while walking from keyframe to target after a seek, only the last `seekWalkCacheWindow` frames (default 4, env `TRACE_SEEK_CACHE_WINDOW`, 0–12) are converted and cached. Each is a full-res conversion (~14ms at 4K) the user waits on before their frame appears — 12 of them added ~170ms to every H.264 scrub landing. Ordinary (non-seek) forward walks still use the full 12. HUD `walk Nf cache Ncv/Nms` shows the cost.
