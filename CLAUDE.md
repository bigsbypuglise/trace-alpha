...
3. Reliable reverse playback beyond the frame cache (ProRes is cheap — every frame is a keyframe; H.264 needs GOP-aware backward buffering)
4. EXR / image-sequence review polish, OCIO display transform (TODO marker in `StillImageLoader.cpp`). **EXR does not open today**: OpenImageIO is not installed in vcpkg and not built in CI, so `TRACE_WITH_OIIO` is undefined in both.
