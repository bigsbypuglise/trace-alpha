...
- **Reverse cache is sized by cost, not by frame count** (Aug 2026, supersedes the fixed 4-frame window): capacity is set at open from frame footprint (~192MB budget → 7 frames at 4K, 32 at 1080p), and the seek-walk fill window is whatever fits an ~18ms conversion budget using measured `avgConvertMs`. A fixed count is wrong in both directions: at 0.7ms/frame (1080p H.264) caching a lot is nearly free and saves whole GOP re-walks on backward stepping, while at 14ms/frame (4K) each entry is latency the user feels on the landing frame. The Aug 2026 fixed window of 4 fixed scrub landing but made repeated backward stepping seek ~3x more often. Env `TRACE_SEEK_CACHE_WINDOW` still forces a count; HUD `walk Nf cache Ncv/Nms` shows the cost.
...
1. **4K ProRes 4444 playback is at the limit of the sync design** (2026-08-04): the July "smooth" validation was ProRes **422** (dec ~1ms, cvt ~14ms). A 4K ProRes **4444** file (`yuva444p12le` — 12-bit, 4:4:4, alpha) measures dec ~13ms + cvt ~25ms = ~38ms against a 41.7ms budget at 24fps, i.e. ~91% of frame time on the UI thread, so playback can't hold rate. Alpha stripping (above) removes some conversion cost, but 4:4:4 12-bit is inherently ~2x the chroma of 422. If that isn't enough, the honest options are (a) the async decode/convert pipeline that was reverted twice — done properly with sequence-numbered requests, (b) GPU conversion, or (c) accepting reduced playback rate on 4444 while keeping stepping exact. Don't reach for read-ahead queues again.
2. ~~Frame-exact scrub/step on long-GOP H.264~~ **Validated 2026-07-30**: exactness correct, landing latency was slightly slow (<0.5s). Trimmed the speculative cache fill (see decision above) — **awaiting re-validation**. If still slow, remaining levers in order: (a) slice-only threading for H.264 in Step mode (avoids ~100ms frame-thread pipeline refill; needs codec reconfigure), (b) cap the exact-decode walk length, (c) half-res en-route conversions.
