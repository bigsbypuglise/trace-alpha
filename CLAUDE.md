...
3. **GPU-backed presentation / D3D11 — GATE B complete (2026-08-09).** The plan and every decision live in `docs/gpu-initiative-plan.md`; read that first, it wins over anything summarised here. Steps 2–6 are committed and validated: `VideoFrame` replaced bare `QImage` at the four frame seams (`03d840e`), the `VideoRenderer` boundary exists (`5765c19`), generation plumbing landed (`75a3412`), random-access scrub decode runs on a worker with latest-target-wins (`ff55d4e` + `f77d472`, plan §14), and **there is now a native D3D11 surface** (plan §17) — a DXGI flip-model swapchain on a child HWND, presenting swscale's BGRA. **Opt-in via `TRACE_RENDERER=d3d11`; `cpu` is still the default and stays so until GATE E.** Playback and scrub measure identical to the CPU baseline (4K H.264 98.3% either way; reversals `rev-hit 97.9%`, `stalls 2 of 394` vs `2 of 402`), which is the requirement — GATE B is about a native surface being *correct*, not faster. Next is step 7, **GATE C**: planar YUV upload and shader colour conversion, which is where conversion cost actually moves. The exact-cadence problem still belongs to **GATE E** (`docs/gpu-initiative-plan.md §8` item 11): video presents one frame per timer tick, so the rate ceiling is `1000/round(1000/fps)`, and closing it needs a real presentation clock (vsync / waitable swapchain), not another scheduler experiment. **GATE B is harness-validated only — the owner has not looked at the rendered picture**, and for a rendering change that gap matters more than usual: the numbers say the right frame at the right size, not that it looks right.

   Two things from GATE B worth not re-deriving. **`WA_PaintOnScreen` plus a null `paintEngine()` also works on Windows** — it was measured, not assumed; the child HWND was preferred because that attribute is documented X11-only and because a child window leaves Qt's widget model untouched. And **the 1080p validation clip opens on a black frame**, which caused a working D3D11 path to be rebuilt on a wrong theory (plan §17.2). Check a black picture against `Splash_1.mp4` before concluding anything from it; to separate "not presenting" from "presenting black", clear the back buffer to a colour.
